...
/**
 * @param {*} input String that will be encoded.
 * @return {String} Encode the base64.
 */
...
/**
 * @param {*} input Base64 to be decoded.
 * @return {String} Decode the base64.
 */
...
exports.saveLastMessage = functions.firestore
  ...
  .onCreate((change, context) => {
    ...
    /**
     * Obs:
     * You need to get the chat document to access the users
     * of a specific conversation and also the message document for
     * have access to the text of the last message.
     * Then save yourself and this text as the last message.
     */
      ...
      chatRef.onSnapshot(snapChat => {
        ...
        chatRef
          ...
          .onSnapshot(snapMessage => {
            ...
            // references to the owner of the message and the recipient of the message.
            let userFrom = messageDoc.from;
            let userTo = Object.keys(chatDoc.users).filter(
              key => key !== encode64(userFrom)
            )[0];
