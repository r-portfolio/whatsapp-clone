...
export class Message extends Model {
  ...
  /**
   *  Returns an instance of the collection`messages`.
  ...
  /**
   * Sends an image to Storage..
   * @param { String } chatId  Chat ID. "Where".
   * @param {*} from  Who is sending. "From where".
   * @return { Promise } Picture message. "Content".
   */
  static sendImage(chatId, from, file) {
    return new Promise((s, f) => {
      Message.upload(file, from)
        .then(downloadFile => {
          Message.send(chatId, from, "image", downloadFile)
            .then(() => {
              s();
            })
            .catch(err => {
  ...
  /**
   * Sends a document from a specified user.
   * @param { String } chatId Chat ID. "Where".
   * @param {*} from Who is sending. "From where".
   * @param { String } file Document to be sent.
   * @param {*} preview Preview the document.
   * @param {*} info Document Information.
   * @return { Promise } Preview of uploaded file.
   */
  ...
  /**
   * Send a contact.
   * @param {*} chatId Chat ID. "Where".
   * @param {*} from Who is sending. "From where".
   * @param {*} contact Contact to be sent.
   ...
   * @param {*} photo  Image of the current user.
   * @return { Promise } Audio sent.
   */
  ...
  /**
   * Sends a message from a specified user.
   * @param { String } chatId Chat ID. "Where".
   * @param {*} from Who is sending. "From where".
  ...
  /**
   * Returns a message view according to the message type.
   * @param { Boolean } me Owner of the message. Default = true.
   * @return { HTMLDivElement } Div with the message view.
   */
  getViewElement(me = true) {
    ...
    switch (this.type) {
      ...
      case "audio":
        ...
        // manipulating audio player events
        ...
        // manipulating the visual elements of the audio player
        btnPlay.on("click", e => {
          audioEl.play();
        });
    ...
    // verified whether the message is of the type "sent" or "received"
  ...
  /**
   * Returns an element with the status icon.
   * - Status types: wait, sent, received, and read.
